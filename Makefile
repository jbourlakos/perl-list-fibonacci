# This Makefile is for the List::Fibonacci extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/List/Fibonacci.pm]
#     AUTHOR => [q[Ioannis Bourlakos <jbourlakos@gmail.com>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => { ExtUtils::MakeMaker=>q[0] }
#     LICENSE => q[artistic_2]
#     MIN_PERL_VERSION => q[5.006]
#     NAME => q[List::Fibonacci]
#     PREREQ_PM => { Test::More=>q[0] }
#     TEST_REQUIRES => { Test::More=>q[0] }
#     VERSION_FROM => q[lib/List/Fibonacci.pm]
#     clean => { FILES=>q[List-Fibonacci-*] }
...
MAN3EXT = 3pm
INSTALLDIRS = site
INSTALL_BASE = /home/jbourlakos/perl5
DESTDIR = 
PREFIX = $(INSTALL_BASE)
...
MAKE = make
FULLEXT = List/Fibonacci
BASEEXT = Fibonacci
PARENT_NAME = List
DLBASE = $(BASEEXT)
VERSION_FROM = lib/List/Fibonacci.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
TO_INST_PM = lib/List/Fibonacci.pm \
	lib/List/Fibonacci/Iterator.pm
...
DISTNAME = List-Fibonacci
DISTVNAME = List-Fibonacci-0.01
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) '  - '\''Ioannis Bourlakos <jbourlakos@gmail.com>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: List-Fibonacci' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '      "Ioannis Bourlakos <jbourlakos@gmail.com>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "List-Fibonacci",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="List-Fibonacci" VERSION="0.01">' > List-Fibonacci.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> List-Fibonacci.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Ioannis Bourlakos &lt;jbourlakos@gmail.com&gt;</AUTHOR>' >> List-Fibonacci.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> List-Fibonacci.ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,006,0,0" />' >> List-Fibonacci.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi-5.26" />' >> List-Fibonacci.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> List-Fibonacci.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> List-Fibonacci.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> List-Fibonacci.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'lib/List/Fibonacci.pm' 'blib/lib/List/Fibonacci.pm' \
	  'lib/List/Fibonacci/Iterator.pm' 'blib/lib/List/Fibonacci/Iterator.pm' 
